...
add_EV.names = {'wm', 'wp', 'beta_m', 'beta_p', 'LLE', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [params, betas, modelfit] = function_add_EV(data,num_it); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        add_EV.params(i,:) = [params, betas, modelfit, bic, aic]; 
        toc
end 

    save('//Users/marlene/Desktop/Masterarbeit/ma_code/vbm/results/add_EV', 'add_EV');
