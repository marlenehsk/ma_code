% function ml_function_hyb_EU_bayes

function [fitparams, betas, modelfit] = ml_function_hyb_EU_bayes(data, num_it,subpls)
...
% turn objective into subjective values, magnitude scaled by lambda, prob same 
...
svl = wsum * ((1-beta_mult)*(beta_m*sml + beta_p*subpl) + beta_mult*(subpl.*sml));
svr = wsum * ((1-beta_mult)*(beta_m*smr + beta_p*subpr) + beta_mult*(subpr.*smr));
