% run ml_function_add_SU_bayes 
% ready to run on HPC server 

function ml_run_add_SU_bayes(nname)
...
ml_add_SU_bayes.names = {'wm', 'wp', 'lambda', 'gamma', 'beta_m', 'beta_p', 'modelfit', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [params, betas, modelfit] = ml_function_add_SU_bayes(data, num_it,subpls); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        ml_add_SU_bayes.params(i,:) = [params, betas, modelfit, bic, aic]; 
        toc
end 

    save('./ml_add_SU_bayes', 'ml_add_SU_bayes');
