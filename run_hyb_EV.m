...
hyb_EV_neu.names = {'wm', 'wp', 'wmult', 'beta_mult', 'beta_m', 'beta_p' 'LLE', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [params, betas, modelfit] = function_hyb_EV(data,num_it); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        hyb_EV_neu.params(i,:) = [params, betas, modelfit, bic, aic]; 
        toc 
end 

    save('//Users/marlene/Desktop/Masterarbeit/ma_code/vbm/results/hyb_EV_neu', 'hyb_EV_neu');
