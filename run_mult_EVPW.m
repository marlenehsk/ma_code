% script to run function_mult_EVPW
...
mult_EVPW.names = {'wmult', 'gamma', 'modelfit', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [fitparams, modelfit] = function_mult_EVPW(data,num_it); 
        bic = 2*modelfit + numel(fitparams)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(fitparams)); 
        mult_EVPW.params(i,:) = [fitparams, modelfit, bic, aic]; 
        toc
end 

    save('//Users/marlene/Desktop/Masterarbeit/ma_code/vbm/results/mult_EVPW', 'mult_EVPW');
