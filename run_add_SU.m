...
add_SU.names = {'wm', 'wp', 'lambda', 'gamma', 'beta_m', 'beta_p', 'modelfit', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [params, betas, modelfit,] = function_add_SU(data,num_it); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        add_SU.params(i,:) = [params, betas, modelfit, bic, aic]; 
        toc
end 

    save('//Users/marlene/Desktop/Masterarbeit/ma_code/vbm/results/add_SU', 'add_SU');
