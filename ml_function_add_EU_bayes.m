% function ml_function_add_EU_bayes

function [fitparams, betas, modelfit] = ml_function_add_EU_bayes(data, num_it,subpls)

% function script for add EU model (magnitude, but no prob distortion)

% first: set up fmincon and betas 

lb = [0 0 0]; 
ub = [60 60 3]; % parameters are wm, wp, and lambda


for i = 1:num_it

    x0 = [ub(1)*rand(1) ub(2)*rand(1) ub(3)*rand(1)]; % 3 values 
    A = []; b = []; Aeq = []; beq = []; 
    options = optimoptions('fmincon', 'Display', 'off');

    [fitparams_1, modelfit_1] = fmincon(@EU_model_add, x0, A, b, Aeq, beq, lb, ub, [], options);
...
function loglikehood = EU_model_add(params)
...
svl =(wm*sml)+(wp*sub_pl);
svr =(wm*smr)+(wp*sub_pr);
