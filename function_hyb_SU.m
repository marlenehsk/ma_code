function [fitparams, betas, modelfit] = function_hyb_SU(data, num_it)

% function script for hybrid SU model (magnitude, but no probability
% distortion)

%  first: set up fmincon and betas

lb = [0 0 0 0 0]; 
ub = [60 60 60 3 3]; % parameters are wm, wp, wmult, lambda and gamma
...
for i = 1:num_it
    ...
    [fitparams_1, modelfit_1] = fmincon(@SU_model_hyb, x0, A, b, Aeq, beq, lb, ub, [], options);
