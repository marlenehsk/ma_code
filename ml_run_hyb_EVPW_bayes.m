% run ml_function_hyb_EVPW_bayes 
% ready to run on HPC server 

function ml_run_hyb_EVPW_bayes(nname)
...
ml_hyb_EVPW_bayes.names = {'wm', 'wp', 'wmult', 'gamma', 'beta_m', 'beta_p', 'betamult', 'modelfit', 'LLE', 'bic'}; 
...
for i = 1:numel(vp)
        ...
        [params, betas, modelfit] = ml_function_hyb_EVPW_bayes(data,num_it,subpls); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        ml_hyb_EVPW_bayes.params(i,:) = [params, betas, modelfit, bic, aic]; 
        toc
end 

    save('./ml_hyb_EVPW_bayes', 'ml_hyb_EVPW_bayes');
