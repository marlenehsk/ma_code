...
hyb_EU_neu.names = {'wm', 'wp', 'wmult', 'lambda', 'beta_mult', 'beta_m', 'beta_p', 'LLE', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [params, betas, modelfit] = function_hyb_EU(data,num_it); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        hyb_EU_neu.params(i,:) = [params, betas, modelfit, bic, aic]; 
        toc
end 

    save('//Users/marlene/Desktop/Masterarbeit/ma_code/vbm/results/hyb_EU_neu', 'hyb_EU_neu');
