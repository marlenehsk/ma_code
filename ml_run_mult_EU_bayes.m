% run ml_function_mult_EU_bayes
% ready to run on HPC server 

function ml_run_mult_EU_bayes(nname)
...
ml_mult_EU_bayes.names = {'wmult', 'lambda', 'modelfit', 'bic', 'aic'}; 
...
for i = 1:numel(vp)
        ...
        [params, modelfit] = ml_function_mult_EU_bayes(data, num_it,subpls); 
        bic = 2*modelfit + numel(params)*log(size(data,1)); 
        aic = 2*modelfit + (2*numel(params)); 
        ml_mult_EU_bayes.params(i,:) = [params, modelfit, bic, aic]; 
        toc

end 

   save('./ml_mult_EU_bayes', 'ml_mult_EU_bayes');
